...
public class GraphModel extends Model {
	...
	/**
	 * @hint Caches the result in networkVerticesCount and the offsets of the SubCircuits in
	 *       SubCircuit.offset.
	 * @param offset
	 *           The network number of the first object in this model.
	 * @return returns the number of functions and latches of this model plus all its sub models.
	 */
	...
	/**
	 * @hint Do not change, add or remove any object (function, latch, sub-circuit, etc.) in the
	 *       BLIF-structure as long as you work with the flow network!
	 * @hint The mapping from networking numbers to BLIF-objects is not injective, due to the ability
	 *       of using the same model (sub-circuit) several times in the project.
	 * @return returns a FlowNetwork including sub-models. Use getObjectByNetworkNumber(i) to get the
	 *         BLIF-object, that is linked with the network number i.
	 */
	...
	/**
	 * matches unsolved links: - uses the CubCircuit's relations to link links in the sub model with
	 * links in the parent model - moves solves network numbers along the link paths
	 * 
	 * @param fn
	 * @param h
	 * @return returns true, if at least one more linkage was found in current iteration
	 */
	private boolean convertUnsolvedLinks(FlowNetwork fn, HirachyItem h) {
		if (this != h.model)
			throw new NullPointerException(); // this must not happen
		boolean r = false;
		for (int i = 0; i < h.subHirachies.size(); i++) {
			...
			for (int j = 0; j < hs.unsolvedLinks.size(); j++) {
				...
				for (int k = 0; k < h.unsolvedLinks.size(); k++) {
					...
					if (pl.n1 != -1 && pl.n2 != -1)
						fn.addEdge(new FlowEdge(pl.n1, pl.n2, 1.0));
				}
				// add solved links
				if (ul.n1 != -1 && ul.n2 != -1)
					fn.addEdge(new FlowEdge(ul.n1, ul.n2, 1.0));
			}
			if (hs.model.convertUnsolvedLinks(fn, hs))
				r = true;
		}
		return r;
	}
	 ...
	 * @param o
	 * @param h
	 * @return returns the number of the network vertex, that corresponds to o in contect h
	 */
	private int getNetworkNumberByObject(Object o, HirachyItem h) {
		...
		if (o instanceof String && h.parent == null) {
			...
			for (int i = 0; i < this.inputs.size(); i++)
				if (this.inputs.get(i) == o)
					return i;
			for (int i = 0; i < this.outputs.size(); i++)
				if (this.outputs.get(i) == o)
					return h.offset + this.networkVerticesCount + i;
		}
		if (o instanceof BinFunction)
			return this.functions.indexOf(o) + h.offset;
		if (o instanceof Latch)
			return this.latches.indexOf(o) + this.functions.size() + h.offset;
		return -1;
	}

	/**
	 * Adds edges within the same model as network edges to fn and links to other models as unsolved
	 * links to h
	 * 
	 * @param fn
	 * @param h
	...
	private void addEdge(FlowNetwork fn, HirachyItem h, Object o1, Object o2) {
		...
		if (ul.n1 != -1 && ul.n2 != -1)
			fn.addEdge(new FlowEdge(ul.n1, ul.n2, 1));
		else
			h.unsolvedLinks.add(ul);
	}
	...
	private void addEdges(FlowNetwork fn, HirachyItem h) {
		...
		for (int i = 0; i < this.functions.size(); i++) {
			...
			for (int j = 0; j < o2.numInputs(); j++)
				this.addEdge(fn, h, o2.in().get(j), o2);
		}
	...
	/**
	 * @hint More then one different number can reference the same object, because sub-circuits can
	 *       be used several times in the project.
	 * @param i
	 *           the network vertex number
	 * @return the object, which is associated with this number; this can either be a String
	 *         (=model's input or output param), a BinFunction or a Latch.
	 */
	public Object getObjectByNetworkNumber(int i) {
		if (i < this.inputs.size())
			return this.inputs.get(i);
		if (i < this.inputs.size() + this.networkVerticesCount)
			return this.getObjectByNetworkNumber(i, this.inputs.size());
		if (i < this.inputs.size() + this.networkVerticesCount + this.outputs.size())
			return this.outputs.get(i - this.networkVerticesCount - this.inputs.size());
		return null; // not found
	}
	private Object getObjectByNetworkNumber(int i, int offset) {
		// ignore inputs and outputs in sub-circuits!
		if (i - offset < this.functions.size())
			return this.functions.get(i - offset);
		else
			offset += this.functions.size();
		if (i - offset < this.latches.size())
			return this.latches.get(i - offset);
		else
			offset += this.latches.size();
		for (int j = 0; j < this.subCircuits.size(); j++) {
			GraphModel m = (GraphModel)this.subCircuits.get(j).subModel;
			if (i - offset < m.networkVerticesCount)
				return m.getObjectByNetworkNumber(i, offset);
			else
				offset += m.networkVerticesCount;
		}
		return null; // not found
	}
	 ...
	 * @return returns a string, that describes the type and the name of the given object o.
	 */
	public String getObjectsStringDescription(Object o) {
		if (o instanceof String) {
			if (this.inputs.contains(o))
				return "input " + (String)o;
			if (this.outputs.contains(o))
				return "output " + (String)o;
		}
		if (o instanceof BinFunction) {
	...
	/**
	 * Stores additional information during the creation of a FlowNetwork. This is necessary, because
	 * of the non-injectivity of the mapping.
	 * 
	 * @author Mitja Stachowiak
	 *
	 */
