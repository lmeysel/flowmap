...
public class GraphModel extends Model {
 ...
 public void printNetwork() {
  ...
  while (it.hasNext()) {
   GraphNode n = it.next();
   String s = n.name() + " (";
   if (n instanceof BinFunction) s += "function";
   else if (n instanceof Latch) s += "latch";
   else if (n instanceof SubCircuit.ToSubcircuit) s += "to '"+((SubCircuit.ToSubcircuit)n).subCktLink.subModel.name()+">"+((SubCircuit.ToSubcircuit)n).subInput.name()+"'";
   else if (n instanceof SubCircuit.FromSubcircuit) s += "from '"+((SubCircuit.FromSubcircuit)n).subCktLink.subModel.name()+">"+((SubCircuit.FromSubcircuit)n).subOutput.name()+"'";
   else if (n instanceof GraphNode.InputNode) s += "input";
   else if (n instanceof GraphNode.OutputNode) s += "output";
   s += ")";
