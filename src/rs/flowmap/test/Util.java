...
public class Util {
	 ...
	 */
	public static void writeDOT(String filename, FordFulkerson ff, MappedFlowNetwork net) {
		try {
			...
			for (int i = 0; i < net.V(); i++)
				wtr.write(i + " [label=<" + (i < (net.V() >> 1) - 1 ? net.getVertexByID(i).getId() : "[" + i + "]") + ">;color=" + (ff.inCut(i) ? "red" : "black") + "]" + br);
