...
public class BinFunction implements GraphNode {
	...
	@Override
	public java.util.Set<GraphNode> out() {
		return this.out;
	} // to be overridden
	@Override
	public void free() {
		if (!out.isEmpty())
			throw new RuntimeException("Cannot free GraphNode while it is used by other nodes!");
		in().clear();
	}
	 ...
	 * 
	 * @return the off-set, which is not (highly) optimized but intersect-free.
	 */
	...
	private void addToOffset(IntersectFreeSet off, Cube c) {
		...
		for (int i = 0; i < this.on.size(); i++) {
			...
			if (!a.isValid())
				continue; // c has no intersection with on[i]
			if (a.equals(c))
				return; // c is completely covered by one existing on[i]
			for (int j = 0; j < c.width; j++)
				if (c.getVar(j) == BinFunction.DC && a.getVar(j) != BinFunction.DC) {
		...
		for (int i = 0; i < this.dc.size(); i++) {
			...
			if (!a.isValid())
				continue; // c has no intersection with dc[i]
			if (a.equals(c))
				return; // c is completely covered by one existing dc[i]
			for (int j = 0; j < c.width; j++)
				if (c.getVar(j) == BinFunction.DC && a.getVar(j) != BinFunction.DC) {
					// split into smaller cubes and try again...
					Cube c1 = c.clone();
	...
	/**
	 * Complementation: Computes the off-set from on-set and dc-set by applying DeMorgan's law
	 * (Extremely slow)
	 * 
	 * @return The off-set
	 * @throws Exception
	...
	/**
	 * Computes recursively the disjunctive, expanded function of src' i.E. (ab'c + ac+ a'bc')' -->
	 * (a'+b+c') & (a'+c') & (a+b'+c) --> c'b' + c'a + c'a'b' + bc'a + ba'c + ba'c + a'c'b' Do first
	 * call with impl = 0, prod = DC DC DC..., dst = {}
	 * 
	 * @param c
		...
		for (int i = 0; i < this.on.size(); i++) {
			...
			if (eqMap)
				...
				for (int j = 0; j < this.numInputs(); j++)
					m.setVar(map[j], c.getVar(j));
			}
			if (!foreign.on().covers(m, null, new Set.ForeignCoverer() {
				@Override
				public boolean isCovered(Cube c) {
	...
	public int cost() {
		...
		for (int i = 0; i < on.size(); i++)
			r += on.width() - on.get(i).cardinality2();
		return r;
	}
