...
public class Model {
	...
	public byte correctLinkeage() {
		...
		while (it.hasNext()) {
			...
			if (cn instanceof GraphNode.InputNode)
				continue; // inputs and from-subCircuit links should have no inputs 
			...
			for (int i = 0; i < l.size(); i++) {
				...
				if (n instanceof GraphNode.UnknownNode) { // this node is not linked and has to be replaced!
					...
					if (n_ == null) {
						...
						l.set(i, n_); // correct predecessor node found; replace l[i]
				}
			}
		}
		...
		while (it.hasNext()) {
			GraphNode cn = it.next();
			if (cn instanceof GraphNode.OutputNode)
				continue; // outputs and to-subCircuit links should have no outputs 
	...
	public GraphNode getNodeByName(String name) {
		for (int i = 0; i < this.inputs.size(); i++)
			if (this.inputs.get(i).name().equals(name))
				return this.inputs.get(i);
		for (int i = 0; i < this.functions.size(); i++)
			if (this.functions.get(i).name().equals(name))
				return this.functions.get(i);
		for (int i = 0; i < this.latches.size(); i++)
			if (this.latches.get(i).name().equals(name))
				return this.latches.get(i);
		for (int i = 0; i < this.subCircuits.size(); i++) {
			SubCircuit sc = this.subCircuits.get(i);
			for (int j = 0; j < sc.out.size(); j++)
				if (sc.out.get(j).name().equals(name))
					return sc.out.get(j);
		}
		...
		private void internalNext() {
			...
			if (currentList == model.inputs) {
				...
				if (currentPos < currentList.size()) {
					next = ((GraphNode.InputNode)currentList.get(currentPos));
					return;
				} else {
					currentList = model.functions;
					currentPos = -1;
				}
			}
			if (currentList == model.functions) {
				currentPos++;
				if (currentPos < currentList.size()) {
					next = ((BinFunction)currentList.get(currentPos));
					return;
				} else {
					currentList = model.latches;
					currentPos = -1;
				}
			}
			if (currentList == model.latches) {
				currentPos++;
				if (currentPos < currentList.size()) {
					next = ((Latch)currentList.get(currentPos));
					return;
				} else {
					currentList = model.subCircuits;
					currentPos = 0;
				}
			}
			if (currentList == model.subCircuits) {
				if (currentPos < currentList.size()) {
					...
					if (currentItem < inSize)
						next = ((SubCircuit)currentList.get(currentPos)).in.get(currentItem);
					else
						next = ((SubCircuit)currentList.get(currentPos)).out.get(currentItem - inSize);
				} else {
					currentList = model.outputs;
					currentPos = -1;
				}
			}
			if (currentList == model.outputs) {
				currentPos++;
				if (currentPos < currentList.size()) {
					next = ((GraphNode.OutputNode)currentList.get(currentPos));
					return;
				} else {
					next = null;
				}
	...
	protected static class FktListCreator {
		protected List<BinFunction> newList() {
			return new ArrayList<BinFunction>();
		}
	}
	...
	public static class FreeingArrayList<FreeableNode> extends ArrayList<FreeableNode> {
		...
		@Override
		public FreeableNode remove(int i) {
			FreeableNode n = super.remove(i);
			if (n != null)
				((rs.graphnode.FreeableNode)n).free();
			return n;
		}
		@Override
		public FreeableNode set(int i, FreeableNode n) {
			FreeableNode p = super.set(i, n);
			if (p != null && p != n)
				((rs.graphnode.FreeableNode)p).free();
			return p;
		}
		@Override
		public void clear() {
		...
		public boolean removeAll(Collection<?> c) {
			throw new RuntimeException("This operation is not allowed on current list!");
		}
	}
}